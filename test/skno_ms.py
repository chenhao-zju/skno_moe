...
from skno_block_ms import SKNOBlock, MLPNet, PatchEmbed
...
class SKNO(nn.Cell):
    ...
    def construct(self, x: Tensor):
        ...
        '''
        x = self.encoder(x)
        print('ms after encoder:', x[0,0,0,:10])
        ...
        for blk in self.blocks:
            x = blk(x)
            x0 = x.reshape(b, h, w, c)
            print('ms block output:', x.shape, x0.shape, x0[0,0,0,:10])
        b, _, c = x.shape
        x = x.reshape(b, h, w, c)
...
if __name__ == '__main__':
    B, C, H, W = 1, 20, 128, 256
    input_ = Tensor(np.random.randn(B, C, H, W), dtype=mindspore.float32)
    net = SKNO(image_size=(H, W), in_channels=C, out_channels=C, compute_dtype=mstype.float32)
    output, _ = net(input_)
    print(output.shape)
    mindspore.save_checkpoint(net, "/home/bingxing2/ailab/group/ai4earth/haochen/code/skno_moe/test/checkpoint_ms.ckpt")