...
class InverseRealSHT(nn.Cell):
    ...
    def __init__(self, nlat, nlon, lmax=None, mmax=None, grid="lobatto", norm="ortho", csphase=True):
        ...
        pct = Tensor(pct, dtype=mindspore.float32)
        pct = pct.transpose((2, 1, 0))
        self.weights = Parameter(pct[None, None, :], name="weights")
...
class SKNO1D(nn.Cell):
    ...
    def construct(self, x: Tensor):
        ...
        print('before sht: ', x.transpose((0,2,3,1))[0,0,0,:10])
...
class SKNOBlock(nn.Cell):
    ...
    def construct(self, x: Tensor):
        ...
        x = self.layer_norm(x)
        x = self.filter(x)
        # print('after filter: ', x.reshape(1, 16, 32, 768)[0,0,0,:10])
        x = x + res_x
        res_x = x
        ...
        # print('after ffn: ', x.reshape(1, 16, 32, 768)[0,0,0,:10])
        return x



if __name__ == '__main__':
    inputs = Tensor(np.random.randn(1,20,128,256), dtype=mindspore.float32)
    sht = RealSHT(nlat=128, nlon=256, lmax=128, mmax=129)
    mid_feature = sht(inputs)

    isht = InverseRealSHT(nlat=128, nlon=256, lmax=128, mmax=129)
