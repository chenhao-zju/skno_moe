...
NAME='skno_00to20_ddp3_58_0.0005lr_FullTrain'
...
###主节点运行
torchrun --nnodes=3 --node_rank=0 --nproc_per_node=4 --master_addr="${host[1]}" \
...
torchrun --nnodes=3 --node_rank=1 --nproc_per_node=4 --master_addr="${host[1]}" \
            --master_port="29501" --max_restarts=3 \
            train.py --enable_amp --yaml_config=$config_file --config=$config --run_num=$run_num --exp_dir=$LOG_DIR \
