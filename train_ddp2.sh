...
NAME='skno_00to20_ddp2_58_0.0005lr_FullTrain'

LOG_DIR="/home/bingxing2/ailab/group/ai4earth/haochen/logs/${NAME}/"
mkdir -p -- "$LOG_DIR"

source activate chenhao_env
...
###主节点运行
torchrun --nnodes=2 --node_rank=0 --nproc_per_node=4 --master_addr="${host[1]}" \
...
torchrun --nnodes=2 --node_rank=1 --nproc_per_node=4 --master_addr="${host[1]}" \
            --master_port="29501" --max_restarts=3 \
            train.py --enable_amp --yaml_config=$config_file --config=$config --run_num=$run_num --exp_dir=$LOG_DIR \
...
### torchrun --nnodes=3 --node_rank=2 --nproc_per_node=4 --master_addr="${host[1]}" \
###             --master_port="29501" --max_restarts=3 \
###             train.py --enable_amp --yaml_config=$config_file --config=$config --run_num=$run_num --exp_dir=$LOG_DIR \
